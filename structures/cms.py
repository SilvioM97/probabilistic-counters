...
class CountMinSketch(object):
    ...
    def CMM (self, elem):
        j = 0
        e = array.array ("f", (0 for _ in range (self.d)))
        for table, hashval in zip (self.tables, self._hash (elem)):
            sketchCounter = table[hashval]
            noise = (self.n - sketchCounter) / float (self.m - 1)
            e[j] = sketchCounter - noise
            j += 1
        return int (np.median (e).round ())
